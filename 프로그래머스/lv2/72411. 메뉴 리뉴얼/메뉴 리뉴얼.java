...
class Solution {
    public void getCourses(char nextMenu, Set<String> selectedMenus
        ...
        int occurence = (int) orderList.stream().filter(order -> order.containsAll(selectedMenus)).count();
        if(occurence < 2) return;
        int size = selectedMenus.size();
        
        if(courses.containsKey(size)) {
            List<Course> courseList = courses.get(size);
            Course course = new Course(selectedMenus.stream().sorted()
                                       .collect(Collectors.joining("")), occurence);
            Course original = courseList.get(0);
            if(original.occurence < occurence) {
                courseList.clear();
                courseList.add(course);
            } else if(original.occurence == occurence) {
                courseList.add(course);
            }
        }
    ...
    public String[] solution(String[] orders, int[] course) {
        List<Set<String>> orderList = Arrays.stream(orders).map(String::chars)
            .map(charStream 
                 -> charStream.mapToObj((menu -> String.valueOf((char) menu))).collect(Collectors.toSet()))
        ...
        return courses.values().stream().filter(list -> list.get(0).occurence > 0)
            .flatMap(List::stream).map(c -> c.course).sorted().toArray(String[]::new);
    }
}
