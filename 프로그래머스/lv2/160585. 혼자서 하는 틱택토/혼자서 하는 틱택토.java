...
class Solution {
    public int solution(String[] board) {
        ...
        if(formateLine(arr[1][1], arr[0][0], arr[2][2]) || formateLine(arr[2][0], arr[0][2], arr[1][1])) {
            if(arr[1][1] == 'X') lineX++; if(arr[1][1] == 'O') lineY++;
        }
        
        for(int i = 0 ; i < 3 ; i++) {
            if(formateLine(arr[i][0], arr[i][1], arr[i][2])) {
                if(arr[i][0] == 'X') lineX++; if(arr[i][0] == 'O') lineY++;
            }
            if(formateLine(arr[0][i], arr[1][i], arr[2][i])) {
    ...
    public boolean formateLine(char a, char b, char c) {
        return a == b && b == c;
    }
}