...
class Solution {
    ...
    public int solution(int n, String[] data) {
        ...
        char[][] datas = Arrays.stream(data).map(s -> s.toCharArray()).toArray(char[][]::new);
        for(char[] d : datas) 
            condis.add(new Condition(d[0], d[2], d[3], d[4] - '0' + 1));
        for(String str : set) {
...
class Condition {
    ...
    public boolean matches(char[] str) {
        for(int i = 0 ; i < str.length ; i++) {
            if(a == str[i]) return matchCase(i, str);
        }
        return true;
    }
    public boolean matchCase(int i, char[] str) {
        boolean givenCond = (i - dist >= 0           && str[i - dist] == b) 
                         || (i + dist < str.length() && str[i + dist] == b);
    ...
    public boolean result(int i, boolean mode, String str) {
        for(int j = 0 ; j < dist ; j++) {
            if((i + j < str.length() && str[i + j] == b) 
            || (i - j >= 0           && str[i - j] == b)) return mode;
        }
        return !mode;
    }
}