    ...
    for n in range(1, len(comb1) + 1):
        for x in range(1, 501):
            for num in dice[comb1[n - 1]]:
                if x - num >= 0: dp1[n][x] += dp1[n - 1][x - num]
            for num in dice[comb2[n - 1]]:
                if x - num >= 0: dp2[n][x] += dp2[n - 1][x - num]
...
def solution(_dice):
    ...
    for cur_case in combinations(range(len(dice)), len(dice) // 2):
        sub_case = tuple(sorted(list(set(range(len(dice))) - set(cur_case))))
        
        cur_win_num = get_win_num(cur_case, sub_case)
